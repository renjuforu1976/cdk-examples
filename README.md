...
- aws s3 cp "s3://${S3_BUCKET}/ent-svc-scan-results/${IMAGE_NAME}/sast/$SAST_FILE" sast-# Verify SCA scan exists for this digest
- echo ""
...
- aws s3 cp "s3://${S3_BUCKET}/ent-svc-scan-results/${IMAGE_NAME}/sca/$SCA_FILE" sca-results.# Verify SBOM exists for this digest
- echo ""
- echo "--- Checking SBOM ---"
- |
...
SAST_VERY_HIGH=$(cat sast-results.json | jq '[.findings[] | select(.severity >= 5)] | SAST_HIGH=$(cat sast-results.json | jq '[.findings[] | select(.severity == 4)] | length' SAST_MEDIUM=$(cat sast-results.json | jq '[.findings[] | select(.severity == 3)] | length' SAST_LOW=$(cat sast-results.json | jq '[.findings[] | select(.severity <= 2)] | length' SAST_BLOCK=$(cat sast-results.json | jq '[.findings[] | select(.severity >= 4)] | length' echo "SAST Total findings: $SAST_TOTAL"
...
SAST_DETAILS=$(cat sast-results.json | jq -r '.findings[] | select(.severity >= 4) echo "$SAST_DETAILS"
SAST_PASS=false
else
SAST_PASS=true
fi
...
- |
SCA_CRITICAL=$(cat sca-results.json | jq '[.records[]? | select(.vulnerabilities[]?.cvssScore SCA_HIGH=$(cat sca-results.json | jq '[.records[]? | select(.vulnerabilities[]?.cvssScore SCA_MEDIUM=$(cat sca-results.json | jq '[.records[]? | select(.vulnerabilities[]?.cvssScore SCA_BLOCK=$(cat sca-results.json | jq '[.records[]? | select(.vulnerabilities[]?.cvssScore echo "SCA Vulnerable libraries:"
...
SCA_DETAILS=$(cat sca-results.json | jq -r '.records[]? | select(.vulnerabilities[]?.echo "$SCA_DETAILS"
SCA_PASS=false
else
SCA_PASS=true
fi
...
aws sns publish \
--topic-arn "$SNS_TOPIC_ARN" \
--subject "SECURITY GATE FAILED - $IMAGE_NAME - High/Critical Vulnerabilities Found" --message "SECURITY GATE FAILURE - VULNERABILITIES DETECTED
=============================================
